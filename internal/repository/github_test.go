...
func setupGitHubTestServer(t *testing.T, handler http.HandlerFunc) (*GitHubRepository, *httptest.Server) {
	...
	return &GitHubRepository{client: client}, server
}
...
func TestGitHubClient_PostReview(t *testing.T) {
	t.Run("Success", func(t *testing.T) {
		...
		comments := []*models.Comment{{Body: "This is a test comment", Path: "main.go", Position: 5}}
	...
	t.Run("Failure - API Error", func(t *testing.T) {
		...
		comments := []*models.Comment{{Body: "This comment will fail", Path: "main.go", Position: 5}}
		err := client.PostReview(context.Background(), "owner", "repo", 1, comments, "test-commit-id")
		assert.Error(t, err)
	})
}
