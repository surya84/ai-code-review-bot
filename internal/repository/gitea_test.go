...
func setupGiteaTestServer(t *testing.T) (*GiteaRepository, *http.ServeMux, *httptest.Server) {
	...
	return &GiteaRepository{client: client}, mux, server
}
...
func TestGiteaClient_PostReview(t *testing.T) {
	t.Run("Success", func(t *testing.T) {
		...
		comments := []*models.Comment{{Body: "Gitea test comment", Path: "main.go", Position: 10}}
